...
$OutputPath = Join-Path $Here "group_25_live_extract.json"
...
@'
(async function collectGroup25() {
  const groupId = 25;
  const category = { text: "Group 25", route: "/items/25?parent=0", groupId };
  const articleList = [];
  const pages = [];
...
try {
  ...
  Start-Sleep -Seconds 8
  Navigate-Browser $session "$baseUrl/items/25?parent=0"
  $extract = Invoke-BrowserScript $session (Extract-Expression)
  [ordered]@{
    checkedAt = (Get-Date).ToString("o")
    sourceUrl = "$baseUrl/items/25?parent=0"
    extract = $extract
