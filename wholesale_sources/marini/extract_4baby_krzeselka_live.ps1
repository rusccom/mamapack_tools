...
$OutputPath = Join-Path $Here "forbaby_krzeselka_15_live_extract.json"
...
@'
(async function collect4BabyKrzeselka() {
  const groupId = 15;
  const category = { text: "Krzesełka do karmienia", route: "/items/15?parent=10", groupId };
  const articleList = [];
  const pages = [];
...
try {
  ...
  Start-Sleep -Seconds 8
  Navigate-Browser $session "$baseUrl/items/15?parent=10"
  $extract = Invoke-BrowserScript $session (Extract-Expression)
  [ordered]@{
    checkedAt = (Get-Date).ToString("o")
    sourceUrl = "$baseUrl/items/15?parent=10"
    extract = $extract
